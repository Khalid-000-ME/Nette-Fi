#!/usr/bin/env python3
"""
Tool 5: CSV Upload and Processing
Handles CSV file processing and validation for employee payroll data
Integrates with Tool 1 for batch transaction execution
"""

import pandas as pd
import io
import json
from typing import List, Dict, Any, Optional, Tuple
import re
from decimal import Decimal, InvalidOperation

class UploadAgent:
    """
    Specialized agent for processing payroll CSV uploads and data validation
    """
    
    def __init__(self):
        # Updated to match current system tokens including new volatile tokens
        self.supported_tokens = {
            'ETH', 'USDC', 'USDT', 'DAI', 'WETH', 'MATIC', 'SOL', 'AVAX', 'LINK', 'BTC'
        }
        self.required_columns = ['employee_address', 'token', 'amount']  # Simplified for direct transfers
        
    def validate_ethereum_address(self, address: str) -> bool:
        """Validate Ethereum address format"""
        if not address or not isinstance(address, str):
            return False
        
        # Remove whitespace
        address = address.strip()
        
        # Check if it starts with 0x and has correct length
        if not address.startswith('0x') or len(address) != 42:
            return False
            
        # Check if it contains only valid hex characters
        try:
            int(address[2:], 16)
            return True
        except ValueError:
            return False
    
    def validate_token_symbol(self, token: str) -> bool:
        """Validate token symbol"""
        if not token or not isinstance(token, str):
            return False
        return token.upper().strip() in self.supported_tokens
    
    def validate_amount(self, amount: Any) -> Tuple[bool, Optional[float]]:
        """Validate and convert amount to float"""
        try:
            if isinstance(amount, str):
                # Remove any currency symbols or whitespace
                amount = re.sub(r'[^\d.-]', '', amount.strip())
            
            decimal_amount = Decimal(str(amount))
            float_amount = float(decimal_amount)
            
            # Check if amount is positive and reasonable
            if float_amount <= 0 or float_amount > 1000000:  # Max $1M per employee
                return False, None
                
            return True, float_amount
        except (InvalidOperation, ValueError, TypeError):
            return False, None
    
    def process_csv_content(self, csv_content: str) -> Dict[str, Any]:
        """
        Process CSV content and return structured payroll data
        """
        try:
            # Read CSV content
            df = pd.read_csv(io.StringIO(csv_content))
            
            # Normalize column names (remove spaces, convert to lowercase)
            df.columns = df.columns.str.strip().str.lower().str.replace(' ', '_')
            
            # Check required columns
            missing_columns = [col for col in self.required_columns if col not in df.columns]
            if missing_columns:
                return {
                    'success': False,
                    'error': f'Missing required columns: {", ".join(missing_columns)}',
                    'required_columns': self.required_columns,
                    'found_columns': list(df.columns)
                }
            
            # Process each row
            processed_data = []
            validation_errors = []
            
            for index, row in df.iterrows():
                row_errors = []
                
                # Validate employee address
                employee_address = str(row['employee_address']).strip()
                if not self.validate_ethereum_address(employee_address):
                    row_errors.append(f"Invalid Ethereum address: {employee_address}")
                
                # Validate token
                token = str(row['token']).strip().upper()
                if not self.validate_token_symbol(token):
                    row_errors.append(f"Unsupported token: {token}")
                
                # Validate amount
                amount_valid, amount_value = self.validate_amount(row['amount'])
                if not amount_valid:
                    row_errors.append(f"Invalid amount: {row['amount']}")
                
                if row_errors:
                    validation_errors.append({
                        'row': index + 2,  # +2 because pandas is 0-indexed and CSV has header
                        'errors': row_errors
                    })
                else:
                    processed_data.append({
                        'employee_address': employee_address,
                        'token': token,
                        'amount': amount_value,
                        'row_number': index + 2
                    })
            
            # Generate summary statistics
            if processed_data:
                total_amount = sum(emp['amount'] for emp in processed_data)
                token_breakdown = {}
                
                for emp in processed_data:
                    token = emp['token']
                    
                    if token not in token_breakdown:
                        token_breakdown[token] = {'count': 0, 'total_amount': 0}
                    
                    token_breakdown[token]['count'] += 1
                    token_breakdown[token]['total_amount'] += emp['amount']
                
                summary = {
                    'total_employees': len(processed_data),
                    'total_amount_usd': total_amount,
                    'unique_tokens': len(token_breakdown),
                    'token_breakdown': token_breakdown,
                    'average_payment': total_amount / len(processed_data),
                    'payment_range': {
                        'min': min(emp['amount'] for emp in processed_data),
                        'max': max(emp['amount'] for emp in processed_data)
                    }
                }
            else:
                summary = None
            
            # Generate transaction specifications for Tool 1
            transaction_specs = self.generate_transaction_specs(processed_data)
            
            return {
                'success': True,
                'data': processed_data,
                'summary': summary,
                'validation_errors': validation_errors,
                'total_rows_processed': len(df),
                'valid_rows': len(processed_data),
                'error_rows': len(validation_errors),
                'transaction_specs': transaction_specs
            }
            
        except Exception as e:
            return {
                'success': False,
                'error': f'Failed to process CSV: {str(e)}',
                'details': 'Please ensure your CSV file is properly formatted with the required columns.'
            }
    
    def generate_transaction_specs(self, payroll_data: List[Dict[str, Any]]) -> str:
        """
        Generate transaction specifications string for Tool 1
        Format: "100 USDC to 0x123..., 200 DAI to 0x456..."
        """
        if not payroll_data:
            return ""
        
        specs = []
        for payment in payroll_data:
            spec = f"{payment['amount']} {payment['token']} to {payment['employee_address']}"
            specs.append(spec)
        
        return ", ".join(specs)
    
    def generate_netting_analysis(self, payroll_data: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        Analyze netting opportunities for the payroll batch
        """
        if not payroll_data:
            return {'error': 'No payroll data provided'}
        
        # Calculate token flows (simplified for direct payments)
        token_flows = {}
        for payment in payroll_data:
            token = payment['token']
            amount = payment['amount']
            
            # Track outflows (company paying employees)
            if token not in token_flows:
                token_flows[token] = {'out': 0, 'in': 0}
            token_flows[token]['out'] += amount
        
        # Calculate net positions
        net_positions = {}
        for token, flows in token_flows.items():
            net_positions[token] = flows['in'] - flows['out']
        
        # Calculate netting savings
        original_transactions = len(payroll_data)
        
        # Estimate netted transactions (simplified logic)
        tokens_with_net_outflow = sum(1 for net in net_positions.values() if net < 0)
        tokens_with_net_inflow = sum(1 for net in net_positions.values() if net > 0)
        estimated_netted_transactions = max(tokens_with_net_outflow, tokens_with_net_inflow, 1)
        
        # Calculate savings
        gas_per_transaction = 12.50  # USD
        original_gas_cost = original_transactions * gas_per_transaction
        netted_gas_cost = estimated_netted_transactions * gas_per_transaction
        gas_savings = original_gas_cost - netted_gas_cost
        savings_percentage = (gas_savings / original_gas_cost) * 100 if original_gas_cost > 0 else 0
        
        return {
            'original_transactions': original_transactions,
            'netted_transactions': estimated_netted_transactions,
            'gas_savings_usd': round(gas_savings, 2),
            'savings_percentage': round(savings_percentage, 1),
            'original_gas_cost': round(original_gas_cost, 2),
            'netted_gas_cost': round(netted_gas_cost, 2),
            'token_flows': token_flows,
            'net_positions': net_positions,
            'execution_time_estimate': f"{estimated_netted_transactions * 15} seconds",
            'mev_protection': 'Full protection through internal netting'
        }
    
    def validate_csv_format(self, csv_content: str) -> Dict[str, Any]:
        """
        Quick validation of CSV format without full processing
        """
        try:
            # Try to read just the header
            lines = csv_content.strip().split('\n')
            if len(lines) < 2:
                return {
                    'valid': False,
                    'error': 'CSV file must contain at least a header and one data row'
                }
            
            # Check header
            header = lines[0].lower().strip()
            required_cols = ['employee_address', 'token', 'amount']
            
            missing_cols = []
            for col in required_cols:
                if col not in header:
                    missing_cols.append(col)
            
            if missing_cols:
                return {
                    'valid': False,
                    'error': f'Missing required columns: {", ".join(missing_cols)}',
                    'required_columns': required_cols
                }
            
            return {
                'valid': True,
                'rows_detected': len(lines) - 1,
                'columns_detected': len(lines[0].split(','))
            }
            
        except Exception as e:
            return {
                'valid': False,
                'error': f'Invalid CSV format: {str(e)}'
            }
    
    def generate_sample_csv(self) -> str:
        """
        Generate a sample CSV template for users
        """
        sample_data = [
            ['employee_address', 'token', 'amount'],
            ['0xe66aa5e1bc2B6B0f1F685a50463ef0D18efc1b9B', 'USDC', '2500'],
            ['0xd7cB260c7658589fe68789F2d678e1e85F7e4831', 'ETH', '1.5'],
            ['0x21522c86A586e696961b68aa39632948D9F11170', 'DAI', '2800'],
            ['0xa75Cd05BF16BbeA1759DE2A66c0472131BC5Bd8D', 'SOL', '15'],
            ['0x8aa62d370585e28fd2333325d3dbaef6112279Ce', 'AVAX', '100']
        ]
        
        return '\n'.join([','.join(row) for row in sample_data])

# ASI:One Tool Function
def process_payroll_csv(
    csv_content: str = "",
    csv_file_path: str = "",
    generate_sample: bool = False,
    validate_only: bool = False
) -> Dict[str, Any]:
    """
    Process payroll CSV data for batch transaction execution
    
    Args:
        csv_content: CSV content as string with columns: employee_address, token, amount
        csv_file_path: Path to CSV file (alternative to csv_content)
        generate_sample: If True, returns a sample CSV template
        validate_only: If True, only validates format without full processing
    
    Returns:
        Dict containing processed payroll data and transaction specifications
    """
    try:
        print(f"🔧 Tool 5: Processing payroll CSV data...")
        
        agent = UploadAgent()
        
        # Handle file path input
        if csv_file_path and not csv_content:
            try:
                import os
                if not os.path.exists(csv_file_path):
                    return {
                        "success": False,
                        "error": f"CSV file not found: {csv_file_path}",
                        "message": "Please check the file path and ensure the file exists"
                    }
                
                with open(csv_file_path, 'r', encoding='utf-8') as file:
                    csv_content = file.read()
                    print(f"   📁 Loaded CSV from file: {csv_file_path}")
                    
            except Exception as e:
                return {
                    "success": False,
                    "error": f"Failed to read CSV file: {str(e)}",
                    "message": "Please ensure the file is accessible and in CSV format"
                }
        
        elif not csv_content and not csv_file_path:
            return {
                "success": False,
                "error": "Either csv_content or csv_file_path must be provided",
                "message": "Please provide CSV data as content string or file path"
            }
        
        if generate_sample:
            sample_csv = agent.generate_sample_csv()
            print(f"   Generated sample CSV template")
            return {
                "success": True,
                "sample_csv": sample_csv,
                "message": "Sample CSV template generated successfully"
            }
        
        if validate_only:
            validation_result = agent.validate_csv_format(csv_content)
            print(f"   CSV validation: {'✅ Valid' if validation_result.get('valid') else '❌ Invalid'}")
            return {
                "success": validation_result.get('valid', False),
                "validation": validation_result,
                "message": "CSV format validation completed"
            }
        
        # Full processing
        result = agent.process_csv_content(csv_content)
        
        if result.get('success'):
            print(f"✅ Tool 5: CSV processed successfully!")
            print(f"   Valid employees: {result.get('valid_rows', 0)}")
            print(f"   Total amount: ${result.get('summary', {}).get('total_amount_usd', 0):.2f}")
            print(f"   Unique tokens: {result.get('summary', {}).get('unique_tokens', 0)}")
            
            # Generate netting analysis if data exists
            if result.get('data'):
                netting_analysis = agent.generate_netting_analysis(result['data'])
                result['netting_analysis'] = netting_analysis
                print(f"   Estimated gas savings: ${netting_analysis.get('gas_savings_usd', 0)}")
            
            return {
                "success": True,
                "data": result,
                "message": f"Processed {result.get('valid_rows', 0)} employee payments successfully"
            }
        else:
            print(f"❌ Tool 5: CSV processing failed - {result.get('error', 'Unknown error')}")
            return {
                "success": False,
                "error": result.get('error', 'CSV processing failed'),
                "details": result.get('details', '')
            }
            
    except Exception as e:
        error_msg = f"Unexpected error: {str(e)}"
        print(f"❌ Tool 5: {error_msg}")
        return {
            "success": False,
            "error": error_msg
        }

# Tool schema for ASI:One
CSV_UPLOAD_TOOL_SCHEMA = {
    "type": "function",
    "function": {
        "name": "process_payroll_csv",
        "description": "Process CSV files containing employee payroll data for batch transaction execution. Accepts either CSV content as string or file path. Validates addresses, tokens, and amounts, then generates transaction specifications for Tool 1.",
        "parameters": {
            "type": "object",
            "properties": {
                "csv_content": {
                    "type": "string",
                    "description": "CSV content as string with columns: employee_address, token, amount. Each row represents one employee payment. Use this OR csv_file_path.",
                    "default": ""
                },
                "csv_file_path": {
                    "type": "string",
                    "description": "Path to CSV file (e.g., 'payroll.csv', 'C:/data/employees.csv'). Use this OR csv_content.",
                    "default": ""
                },
                "generate_sample": {
                    "type": "boolean",
                    "description": "If true, generates and returns a sample CSV template instead of processing data",
                    "default": False
                },
                "validate_only": {
                    "type": "boolean", 
                    "description": "If true, only validates CSV format without full processing",
                    "default": False
                }
            },
            "required": [],
            "additionalProperties": False
        },
        "strict": True
    }
}

# Example usage and testing
if __name__ == "__main__":
    agent = UploadAgent()
    
    # Test with sample CSV
    sample_csv = agent.generate_sample_csv()
    result = agent.process_csv_content(sample_csv)
    
    print("Sample CSV Processing Result:")
    print(json.dumps(result, indent=2))
    
    if result['success'] and result['data']:
        netting_analysis = agent.generate_netting_analysis(result['data'])
        print("\nNetting Analysis:")
        print(json.dumps(netting_analysis, indent=2))